...
if args.generate_pigdm:
    ...
    print("Generating PiGDM-guided (inpainted) samples...")
    num_samples = 25
    start_time = time.time()
    x, xs = generate_pigdm(
        model,
    ...
    plt.suptitle("PiGDM-inpainted MNIST (top half fixed)")
    plt.tight_layout()
    plt.show()
...
if args.generate_pcfm:
    ...
    print("Generating PCFM-projected (inpainted) samples...")
    num_samples = 25
    start_time = time.time()
    x, xs = generate_pcfm(
        model,
    ...
    plt.suptitle("PCFM-inpainted MNIST (top half fixed)")
    plt.tight_layout()
    plt.show()
